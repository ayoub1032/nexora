...
public class P2PContractViewController {
    ...
    public static class P2PContractRow {
        ...
        public P2PContractRow(long contractId, long creatorId, String creatorName, String assetName, String contractType, int quantity, double pricePerUnit, String status) {
    ...
    private void setupTableColumns() {
        colCreator.setCellValueFactory(cellData -> new javafx.beans.property.SimpleStringProperty(cellData.getValue().getCreatorName()));
        colAsset.setCellValueFactory(cellData -> new javafx.beans.property.SimpleStringProperty(cellData.getValue().getAssetName()));
        colType.setCellValueFactory(cellData -> new javafx.beans.property.SimpleStringProperty(cellData.getValue().getContractType()));
        colQuantity.setCellValueFactory(cellData -> new javafx.beans.property.SimpleObjectProperty<>(cellData.getValue().getQuantity()));
        colPrice.setCellValueFactory(cellData -> new javafx.beans.property.SimpleObjectProperty<>(cellData.getValue().getPricePerUnit()));
    ...
    private void updateTotalPrice() {
        try {
            if (cbAsset.getValue() != null && !tfQuantity.getText().isEmpty()) {
                double price = cbAsset.getValue().getValue();
                int qty = Integer.parseInt(tfQuantity.getText());
                tfPrice.setText(String.format("%.2f", price * qty));
            }
        ...
        try {
            ...
            for (P2PContract contract : openContracts) {
                if (contract.getCreatorId() != userId) { // Don't show own contracts
                    ...
                    String creatorName = "User #" + contract.getCreatorId();
    ...
    public void createContract() {
        try {
            if (cbAsset.getValue() == null || tfQuantity.getText().isEmpty() || cbContractType.getValue() == null) {
            ...
            if ("SELL".equals(contractType)) {
                ...
                if (portfolio != null) {
                    var assetInPortfolio = portfolioAssetService.findByPortfolioAndAsset(portfolio.getPortfolioId(), cbAsset.getValue().getAssetId());
            ...
            // Create contract
            P2PContract contract = new P2PContract(userId, cbAsset.getValue().getAssetId(), quantity, price, contractType);
