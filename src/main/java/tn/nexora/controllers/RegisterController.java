...
public class RegisterController {
    ...
    private void loadStep1() {
        ...
        // Role selection
        VBox roleSelectionBox = new VBox(10);
        Label roleLabel = new Label("ACCOUNT TYPE");
        roleLabel.setStyle(
                "-fx-font-size: 11px; -fx-font-weight: bold; -fx-text-fill: #374151; -fx-letter-spacing: 0.5px;");

        this.roleGroup = new ToggleGroup();

        RadioButton traderRadio = new RadioButton("Normal User (Trader)");
        traderRadio.setToggleGroup(this.roleGroup);
        traderRadio.setSelected(true); // Default selection
        traderRadio.setUserData("TRADER");
        traderRadio.setStyle("-fx-font-size: 14px; -fx-text-fill: #1F2937;");

        RadioButton adminRadio = new RadioButton("Administrator");
        adminRadio.setToggleGroup(this.roleGroup);
        adminRadio.setUserData("ADMIN");
        adminRadio.setStyle("-fx-font-size: 14px; -fx-text-fill: #1F2937;");

        Label roleNote = new Label("ℹ Administrators can view all wallets and manage the system.");
        roleNote.setStyle("-fx-text-fill: #6B7280; -fx-font-size: 12px;");
        roleNote.setWrapText(true);

        roleSelectionBox.getChildren().addAll(roleLabel, traderRadio, adminRadio, roleNote);
        ...
        step.getChildren().addAll(title, subtitle, emailBox, passwordBox, confirmPasswordBox, roleSelectionBox,
                buttonBox);
    ...
    private void createAccount() {
        ...
        String selectedRole = this.roleGroup != null && this.roleGroup.getSelectedToggle() != null
                ? (String) this.roleGroup.getSelectedToggle().getUserData()
                : "TRADER";
        ...
        try {
            // Add user to database and get the user_id
            ...
            System.out.println("✅ Wallet created automatically for user_id: " + userId);
            ...
            alert.setContentText(
                    "Your account has been successfully created.\n\nEmail: " + registrationEmail + "\nName: "
                            + registrationFullName + "\n\n✅ A wallet has been automatically created for you!");
            alert.showAndWait();

            // Navigate to login
            MainFX.showLoginScreen();
        } catch (Exception e) {
            step4Error.setText("❌ Error creating account: " + e.getMessage());
            step4Error.setVisible(true);
            e.printStackTrace();
        }
    }
